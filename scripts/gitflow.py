...
def layout(spec):
    ...
    keys = [event_slot_key(e, i, mode) for i, e in enumerate(spec.get("events", []))]
    unique = []
    for key in keys:
    ...
    for i, event in enumerate(spec.get("events", [])):
        key = keys[i]
        x = slot_x[key] + per_slot[key] * 32
        per_slot[key] += 1
...
def to_drawio(spec, route_mode):
    ...
    page = (
        f'  <diagram id="gitflow" name="{attr(title)}">\n'
...
def main(argv=None):
    ...
    errors, warnings = gitflow_validate.validate_spec(spec)
    ...
    if errors:
        ...
        sys.exit(1)
    try:
        xml, router = to_drawio(spec, args.route)

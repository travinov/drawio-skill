...
исправимую ошибку с той же нормализованной сигнатурой. Пользователь нужен при
неоднозначном semantic conflict, подтверждённом plateau, нарушении целостности
evidence и final acceptance — команда `continue` после каждой обычной попытки
не требуется. `trace` показывает цепочку ролей, фактические модели,
...
а `host-result.json` показывает `model_diversity_degraded: true`. Нарушения

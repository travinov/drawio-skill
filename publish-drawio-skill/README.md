...
Как Gemini/GigaCode-compatible extension папка содержит
`gemini-extension.json` и `agents/*.md`. Для CLI, поддерживающего Gemini
extension contract, сначала проверьте и подключите локальную папку:

```bash
gemini extensions validate /absolute/path/to/publish-drawio-skill
gemini extensions link /absolute/path/to/publish-drawio-skill
```
...
unzip drawio-skill-agent-extension.zip -d ~/.gigacode/skills
```
